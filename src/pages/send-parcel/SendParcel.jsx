...
const SendParcel = () => {
  ...
  const senderDistricts =
    serviceCenters && selectedSenderRegion
      ? [
          ...new Set(
            serviceCenters
  ...
  const receiverDistricts =
    serviceCenters && selectedReceiverRegion
      ? [
          ...new Set(
            serviceCenters
  ...
  if (
    senderDistrict &&
    receiverDistrict &&
    (documentType === "Document" || parcelWeight)
  ) {
    ...
    else if (!isDocument && !isWeightAbove3 && isWithinCity)
      deliveryCharge = 110;
    else if (!isDocument && !isWeightAbove3 && !isWithinCity)
      deliveryCharge = 150;
    else if (!isDocument && isWeightAbove3 && isWithinCity)
      deliveryCharge = weight * 40;
    else if (!isDocument && isWeightAbove3 && !isWithinCity)
      deliveryCharge = weight * 40 + 40;
  }
  ...
  const onSubmit = (data) => {
    ...
    if (data.senderDistrict !== data.receiverDistrict) {
      finalData.sameCity = false;
      finalData.receive_warehouse = `${data.senderDistrict} warehouse`;
      finalData.dispatch_warehouse = `${data.receiverDistrict} warehouse`;
    } else {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex flex-col lg:flex-row gap-8 mb-8 pb-8 border-b border-gray-100">
          ...
          <div className="flex-1">
            <h3 className="text-xl font-bold text-[#0A2533] mb-6">
              Sender Details
            </h3>
        ...
        <div className="mt-4">
          ...
          <div className="bg-gray-50 border border-gray-100 rounded-xl p-6 mb-6 flex justify-between items-center">
            <span className="text-xl font-bold text-[#0A2533]">
              Delivery Charge:
            </span>
